...
When the MCP memory server is available, check it first: if this conversation or an earlier
one may have recorded relevant facts, search the memory graph and ground your answer in them.
When the user states durable personal facts or decisions (specs, settings, preferences,
choices), record them in the memory graph before continuing.
