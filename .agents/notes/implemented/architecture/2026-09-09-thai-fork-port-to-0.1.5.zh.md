...
- **已被取代的功能文件取上游。** 0.1.5 已自带回合导轨（`ui-chat` 的 `TurnNavigator` 与 `turn-rail-items`）、`ui-tool` 的图片卡片、轨迹本地化座位，以及 win32 对话框前台修复；分支中的平行实现直接丢弃，不再合并，其中包括上游 `ui-chat` 界面所取代的聊天面板组件（`PromptNavigator`、`ThinkingDock`、`TurnCompletedSteps`、`scratchpad`）。
- **上游删除的包**（`host/apiproxy`、`client/runtime`、`examples`、`tool-subagent-report`、`acp-snapshot`）按删除接受。原本位于其中的分支功能（MCP 设置管理器、应用内更新 RPC）暂缓，留待后续移植到 Typert Remote 架构；其客户端 UI 与测试一并暂缓。为该管理器供数的 mcp-client 状态存储（`src/status.ts` 以及 `startConnection` 与 `syncTools` 的 sink 参数）出于同样原因被移除，使 `packages/mcp/mcp-client` 与上游完全一致。
