...
- Publishes per-server connection status on the root context as `ctx.mcpStatus` (an `McpStatusStore`): the connection supervisor writes each instance's phase, retry state, error, and discovered tools through `handle(serverName)`, and `snapshot()` returns them serverName-sorted for display. The store is created on first activation, shared by every instance in the app, and never disposed.
