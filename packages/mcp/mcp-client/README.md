...
The supervisor publishes each instance's live phase on the root context as `ctx.mcpStatus` (an `McpStatusStore`): `connecting`, `connected` with the discovered tools, `reconnecting` with the attempt count, delay, and error, and `disabled` when reconnection is off or the attempt budget is exhausted; disposal removes the entry. The store is created on first activation, shared by every instance in the app, and never disposed, so a configuration surface can read a snapshot without coupling to the client's internals.
