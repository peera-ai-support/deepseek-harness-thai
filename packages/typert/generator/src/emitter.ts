...
export class FaceModelEmitter {
  ...
  private renderDts(packageModel: PackageModel, schemas: SchemaArtifact): string {
    ...
    for (const [specifier, names] of [...imports].sort(([left], [right]) => compareArtifactKeys(left, right))) {
  ...
  private renderRemoteDts(packageModel: PackageModel): Pick<RemoteModelEmitResult, 'dts' | 'dtsMap'> {
    ...
    for (const [specifier, values] of [...grouped].sort(([left], [right]) => compareArtifactKeys(left, right))) {
      const names = values.sort((left, right) => compareArtifactKeys(left.local, right.local)).map(value =>
