...
export interface TurnNavigatorProps {
  ...
  /** The Turn whose jump is actively paging history in, if any. */
  readonly busyTurn: number | null
  readonly onNavigate: (item: TurnRailItem) => void
  readonly t: (
    key: 'chat.turnNavigation.label' | 'chat.turnNavigation.jump' | 'chat.turnNavigation.jumpLoad' | 'chat.turnNavigation.turn',
    params?: Record<string, unknown>,
  ) => string
}

/** Distance between mark centers; must match the markPosition CSS layout. */
const TURN_SPACING_PX = 10
/** Inset at each end so the first/last mark centers stay inside the frame. */
const RAIL_INSET_PX = 8
/** Keep the active mark clear of the top/bottom fade masks when following. */
const FADE_PX = 24
 ...
 * page history in first — with hover and focus previews.
 */
