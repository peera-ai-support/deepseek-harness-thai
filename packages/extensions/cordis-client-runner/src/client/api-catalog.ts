...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'register<N extends Extract<keyof LocaleNamespaceMap, string>>( ns: N, dicts: Record<RequiredLocaleId, LocaleDictOf<N>> & Partial<Record<BuiltInLocaleId, Partial<LocaleDictOf<N>>>>, ): () => void',
        description: 'Register a declared namespace\'s dictionaries, all locales in one call — the typed form: each dictionary is checked against the namespace\'s LocaleNamespaceMap key union (a missing or extra key is a compile error), and every locale in REQUIRED_LOCALE_IDS is required (bilingual balance enforced at registration); the remaining shipped locales are optional and fall back through the locale\'s chain. Duplicate (ns, locale) throws (single occupant; a namespace\'s texts have one owner). Registration bumps the revision so mounted outlets pick up late-arriving dictionaries.',
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'RequiredLocaleId',
    declaration: 'export type RequiredLocaleId = typeof REQUIRED_LOCALE_IDS[number];',
  },
  {
    name: 'ScopeOf',
