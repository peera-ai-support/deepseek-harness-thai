...
echo ===================================================
echo   DeepSeek Harness (dsh) - Web Launcher
echo ===================================================
echo.
...
if not exist "apps\web\dist" (
    echo [INFO] Building project assets...
    call pnpm run build
    if %ERRORLEVEL% NEQ 0 (
...
if not exist ".env" (
    ...
    echo Please make sure to add your DEEPSEEK_API_KEY into .env
    echo.
)

set PORT=13080
echo [INFO] Starting DeepSeek Harness Web UI on port %PORT%...
echo [INFO] Open your browser at: http://127.0.0.1:%PORT%
echo.

if exist "%~dp0apps\cli\lib\bin.js" (
    call node "%~dp0apps\cli\lib\bin.js" web --port %PORT%
) else (
